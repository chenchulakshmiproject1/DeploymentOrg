...
import getUnitRecordBasedOnProjectFilter from '@salesforce/apex/channelPartner_InventoryController.getProjectDetailsBasedonSearch';
import getProjectDetails from '@salesforce/apex/channelPartner_InventoryController.getProjectDetails';
import getPicklistValuesDynamically from '@salesforce/apex/channelPartner_InventoryController.getAllPickListVal';
...
export default class ChannelPartner_Inventory extends NavigationMixin(LightningElement) {
    ...
    connectedCallback() {
        ...
        setTimeout(() => {
            ...
            this.getAllPicklistValues();
               this.getCityAndProjectNamePickList();
        }, 300);
    }
    ...
    getCityAndProjectNamePickList() {
        getProjectDetails()
            .then(result => {
                if (result.length > 0) {
                    let uniqueCities = [];
                    result.forEach(item => {
                        if (!uniqueCities.includes(item.City__c)) {
                            uniqueCities.push(item.City__c);
                        }
                    });
                    this.optionsCity = uniqueCities.map(city => ({ label: city, value: city }));
                    this.optionsprojectName = result.map(item => ({ label: item.Name, value: item.Name }));
                }
            })
            .catch(error => {
    ...
    getAllPicklistValues() {
        ...
        getPicklistValuesDynamically({ ObjectByField: this.objByField })
            .then(result => {
                debugger;
                this.zoneListOption = this.mapToLabelValuePair(result['Zone__c']);
            })
            .catch(error => {
                console.log('error fetching picklist options ===> ' + error);
            })
    ...
    SearchUnits() {
        ...
        getUnitRecordBasedOnProjectFilter({city: this.selectedCity, zone: this.selectedZone, FlatBhkId: this.FlatBhkId, projectName: this.selectedProjectName, towerName: this.TowerName})
            .then((result) => {
                debugger;
                if(result != null ){
                    ...
                    if (result.FloorwithunitMapDetails) {
                        for (let key in result.FloorwithunitMapDetails) {
                            ...
                            for (let j = 0; j < result.FloorwithunitMapDetails[key].length; j++) {
                                if (result.FloorwithunitMapDetails[key][j].Floor__r.Tower__r.Tower_Code__c == this.TowerName && result.FloorwithunitMapDetails[key][j].Unit_Type__c == this.FlatBhkId) {

...
import createReceipt from '@salesforce/apex/GenerateReceiptDetailsController.createReceipt';
import getPicklistValues from '@salesforce/apex/GenerateReceiptDetailsController.getPicklistValues';
import checkDuplicate from '@salesforce/apex/GenerateReceiptDetailsController.checkDuplicate';
import fetchRelatedReceipts from '@salesforce/apex/GenerateReceiptDetailsController.fetchRelatedReceipts';
import getAccountValues from '@salesforce/apex/GenerateReceiptDetailsController.getAccountValues';
import closeCurrentTask from '@salesforce/apex/GenerateReceiptDetailsController.closeCurrentTask';
import { CloseActionScreenEvent } from 'lightning/actions';

export default class GenerateReceiptDetails extends LightningElement {
    @api recordId;
    ...
    @track showReceiptData = true;
    fetchReceipts() {
        debugger;
        console.log('Fetching Receipts for Record ID:', this.recordId); // Debugging
        fetchRelatedReceipts({ taskId: this.recordId })
            .then(result => {
                if(result && result.length > 0){
                    this.showReceiptData = true;
                }
                else{
                    this.showReceiptData = false;
    ...
    fetchAccounts() {
        ...
        getAccountValues({ taskId: this.recordId })
            .then(result => {
                debugger;
                if (result) {
    ...
    receiptsExists(){
        closeCurrentTask({ taskId: this.recordId })
        .then((result) => {
            this.closeComponent();
        })
        .catch(error => {
    ...
    checkDuplicateReceipt(taskId, type, uniqueNumber) {
        return new Promise((resolve, reject) => {
            if (uniqueNumber) {
                checkDuplicate({ taskId, type, uniqueNumber })
                    .then(result => {
